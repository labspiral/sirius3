...
{
    /// <summary>
    /// Points marking with synchronization by external pulse counts demo
    /// 외부 펄스 개수 동기화를 이용한 포인트 마킹 데모
    /// </summary>
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // External SYNC signal edge level configuration
            // 외부 동기화 신호의 엣지 레벨 설정
            ...
            else if (rtc is Rtc6 rtc6)
            {
                ...
                // 아래 두 기능 중 하나만 선택하여 사용:
                // ConfigLASER1Synchronization(rtc6);
                // or
                ConfigLASER1Synchronization(rtc6); // RTC6 only / RTC6 전용
            }
        ...
        /// <summary>
        /// Verify if the RTC card is RTC5 or RTC6 (Supports D.IN1 at LASER PORT)
        /// RTC5 또는 RTC6 카드인지 확인 (LASER 포트의 D.IN1 지원 여부)
        /// </summary>
        /// <param name="rtc"></param>
        void VerifyProperCard(IRtc rtc)
        {
            // RTC5 and above provides DIGITAL IN1 pin on the LASER connector
            // RTC5 이상에서만 LASER 커넥터에 있는 DIGITAL IN1 핀이 제공됩니다.
            Debug.Assert(rtc is Rtc5 || rtc is Rtc6);
        }

        /// <summary>
        /// Configure D.IN1 signal edge (falling or rising) for external pulse counting
        /// D.IN1 신호 엣지(상승/하강) 설정 (외부 펄스 개수 카운팅용)
        /// <para>
        ...
        /// </para>        /// </summary>
        /// <param name="rtc"></param>
        /// <param name="isDIGITALIN1FallingEdge"></param>
        void ConfigLASERPORT_DIN1_Pin_SignalLevel(IRtc rtc, bool isDIGITALIN1FallingEdge = true)
        {
            // External SYNC signal must be connected to DIGITAL IN1 pin on LASER connector
            // 외부 펄스 신호는 RTC 보드의 LASER 커넥터에 있는 DIGITAL IN1 핀에 반드시 연결해야 합니다.
        ...
        /// <summary>
        /// Configure Output Synchronization (Sync scanner motion to external clock via D.IN1)
        /// <para>
        ...
        {
            // Sync scanner position based on external laser pulse timing to prevent jitter
            // 외부 레이저 펄스 타이밍에 따라 스캐너 위치를 동기화하여 시작점 떨림(Jitter) 방지
            if (rtc is Rtc5 rtc5)
            {
                var lcs = rtc5.LaserControlSignal;
        ...
        /// <summary>
        /// Configure LASER1 Synchronization (Sync LASER1 signal phase to external clock via D.IN1)
        /// LASER1 동기화 설정 (D.IN1을 통한 외부 클럭에 LASER1 신호 위상 동기화)
  		/// <para>
        ...
        {
            // Perfect phase synchronization with external laser source (RTC6 only)
            // 외부 레이저 소스와의 완벽한 위상 동기화 (RTC6 전용)
            // Note: Do not use with OutputSynchronization simultaneously
            // 주의: OutputSynchronization과 동시에 사용하는 것은 금지됩니다.
            rtc6.CtlLASER1Synchronization(true, 0);
        }

        /// <summary>
        /// Create test entities and initialize pen values
        /// 테스트 엔티티 생성 및 펜 파라메터 초기화
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void BtnCreateEntities_Click(object sender, EventArgs e)
        {
            ...
            // Create massive points
            // 대량의 포인트 생성
            CreateEntities();

            // Configure pen for sync pulses
            // 펄스 동기화를 위한 펜 설정
            EditPenValues();
        }

        /// <summary>
        /// Create a large point cloud and optimize its path
        /// 대량의 포인트 클라우드 생성 및 경로 최적화
        /// </summary>
        void CreateEntities()
        {
            var document = siriusEditorControl1.Document;
            ...
            // 경로 최적화 적용 (TSP 알고리즘 사용)
            points.Sort( EntityPoints.PointSorts.Global);

            var color = SpiralLab.Sirius3.UI.Config.EntityPenColors[0]; // Color.White / 흰색 펜
        ...
        /// <summary>
        /// Configure pen parameters for pulse-count based marking
        /// 펄스 개수 기반 마킹을 위한 펜 파라메터 설정
        /// </summary>
        void EditPenValues()
        {
            var document = siriusEditorControl1.Document;
            ...
            // Use Jump and Shoot mode
            // 점프 및 가공(Jump and Shoot) 모드 사용
            entityPen.RasterMode = RasterModes.JumpAndShoot;

            // Pixel time: Max wait (timeout) time for each dot (usec)
            // 최대 대기(타임아웃) 시간 설정 (usec)
            entityPen.PixelTime = 100;

            // Configure external pulse counting:
            // 외부 펄스 카운팅 설정:
            // If PixelPulses > 0:
            //   RTC waits for specified number of edges from D.IN1 on LASER connector
            //   Scanner position is fixed during execution to mark precise dots.
            // PixelPulses > 0 인 경우:
            //   LASER 커넥터의 D.IN1 핀으로 들어오는 신호 엣지 개수를 세어 동기화된 펄스 출력
            //   명령 실행 중 스캐너 위치는 고정되어 정밀한 점(Dot) 가공에 사용됨
            ...
            //   외부 펄스 신호는 반드시 LASER 커넥터의 DIGITAL IN1 핀에 TTL 레벨로 공급되어야 하고, IRtcSignalLevel.CtlLaserControlSignal 을 이용해 상승(혹은 하강) 에지를 설정할 수 있습니다.             entityPen.PixelPulses = 5;

            // Break waiting immediately if pulse count is reached
            // 목표 펄스 개수 도달 시 즉시 대기 종료 및 다음 명령 수행
            entityPen.IsPixelPulsesExit = true;

            // Optional: Hard Jump configuration
            // 선택 사항: 하드 점프(Hard Jump) 설정
            // Note: Hard Jump moves to target directly without microsteps, bypassing mechanical settling time.
            // 참고: 하드 점프는 마이크로스텝 분할 없이 목표 위치로 직접 이동하며 기계적 진동 대기 시간을 생략함.
