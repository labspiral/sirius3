...
namespace Demos
{
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            // Create devices manually (or can be created by EditorHelper.CreateDevices)
            ...
            Debug.Assert(SpiralLab.Sirius3.Core.IsRunningPlatform64);
            ...
            double maxPowerWatt = 20;
            var laser = LaserFactory.CreateVirtual(0, maxPowerWatt, PowerControlMethods.Unknown);
            ...
            // Default power as 10 %
            success &= laser.CtlPower(maxPowerWatt * 0.1);
            Debug.Assert(success);
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.S:
                        if (rtc.CtlGetStatus(RtcStatus.Busy))
                            Console.WriteLine("syncaxis is busy now ...");
                        else
                            Console.WriteLine("syncaxis is not busy ...");
                        if (!rtc.CtlGetStatus(RtcStatus.NoError))
                            Console.WriteLine("syncaxis has error(s)");
                    ...
                    case ConsoleKey.F3:
                        //rtc.BandWidth = 2.0f;
                        //rtc.Head1Offset = new Offset(DVec2.Zero, 0);
                        //rtc.Head2Offset = 
                        //rtc.Head3Offset = 
                        //rtc.Head4Offset = 
        ...
        static bool DrawSquare(IRtc rtc, ILaser laser, MotionTypes motionType, float size = 40)
        {
            ...
            success &= rtc.ListJumpTo(new DVec2(-size / 2.0f, size / 2.0f));
            success &= rtc.ListMarkTo(new DVec2(size / 2.0f, size / 2.0f));
            success &= rtc.ListMarkTo(new DVec2(size / 2.0f, -size / 2.0f));
            success &= rtc.ListMarkTo(new DVec2(-size / 2.0f, -size / 2.0f));
            success &= rtc.ListMarkTo(new DVec2(-size / 2.0f, size / 2.0f));
        ...
        static bool DrawCircle(IRtc rtc, ILaser laser, MotionTypes motionType, float radius = 20)
        {
            ...
            //success &= rtc.ListFrequency(50 * 1000, 2);
            //success &= rtc.ListSpeed(100, 100);
            success &= rtc.ListJumpTo(new DVec2(radius, 0));
            success &= rtc.ListArcTo(DVec2.Zero, 360.0);
            success &= rtc.ListJumpTo(DVec2.Zero);
            if (success)
                success &= rtc.ListEnd();
            if (success)
        ...
        static bool DrawScannerCalibration(IRtc rtc, ILaser laser, int grids = 5, float fieldSize = 50, float velocity = 200)
        {
            ...
            for (int row = 0; row < grids; row++)
            {
                for (int col = 0; col < grids; col++)
                {
                    ...
                    rtc.MatrixStack.Push(DMat4.CreateTranslation(center.X, center.Y, 0));
                    success &= rtc.ListJumpTo(new DVec2(dotSize / 2.0f, 0));
                    success &= rtc.ListArcTo(new DVec2(0, 0), 360);
                    rtc.MatrixStack.Pop();
                    if (!success)
                        break;
                }
        ...
        static bool DrawOptimizeLaserDelay(IRtc rtc, ILaser laser, float vScanner = 500, int numberOfGridPositions = 11, float gridFactor = 2.5f)
        {
            ...
            for (int x = 0; x < numberOfGridPositions; ++x)
            {
                ...
                for (int y = 0; y < numberOfGridPositions; ++y)
                {
                    ...
                    success &= rtcSyncAxis.ListBegin(MotionTypes.ScannerOnly);
                    offset = new DVec2(offset.X, gridFactor * size * y + offsetInitial.Y);
                    /*
                     *  +
                     *  
                     *  L       ---     ---     ---      ---  
                     ...
                     *  -
                     */
                    rtc.MatrixStack.Push(DMat4.CreateTranslation(offset.X, offset.Y, 0));
                    success &= rtc.ListJumpTo(new DVec2(-size / 2, -size));
                    success &= rtc.ListMarkTo(new DVec2(-gap / 2, -size));
                    success &= rtc.ListJumpTo(new DVec2(gap / 2, -size));
                    success &= rtc.ListMarkTo(new DVec2(size / 2, -size));
                    success &= rtc.ListJumpTo(new DVec2(0, -size));
                    success &= rtc.ListMarkTo(new DVec2(0, size));
                    success &= rtc.ListJumpTo(new DVec2(size / 2, size));
                    success &= rtc.ListMarkTo(new DVec2(gap / 2, size));
                    success &= rtc.ListJumpTo(new DVec2(-gap / 2, size));
                    success &= rtc.ListMarkTo(new DVec2(-size / 2, size));
                    success &= rtc.ListJumpTo(new DVec2(-size / 2 - 0.001f, size));
                    rtc.MatrixStack.Pop();
                    success &= rtc.ListJumpTo(DVec2.Zero);
                    success &= rtc.ListEnd();
                    if (success)
        ...
        static bool DrawCheckCalibration(IRtc rtc, ILaser laser, int grids = 5, float fieldSize = 60, float vStage = 20)
        {
            ...
            for (int row = 0; row < grids; row++)
            {
                ...
                for (int col = 0; col < grids; col++)
                {
                    ...
                    rtc.MatrixStack.Push(DMat4.CreateTranslation(center.X, center.Y, 0));
                    success &= rtc.ListJumpTo(new DVec2(-dotSize / 2.0f, 0));
                    success &= rtc.ListMarkTo(new DVec2(dotSize / 2.0f, 0));
                    rtc.MatrixStack.Pop();
                }
            }
            // +  +  +  +  +
            ...
            for (int col = 0; col < grids; col++)
            {
                ...
                for (int row = 0; row < grids; row++)
                {
                    ...
                    rtc.MatrixStack.Push(DMat4.CreateTranslation(center.X, center.Y, 0));
                    success &= rtc.ListJumpTo(new DVec2(0, -dotSize / 2.0f));
                    success &= rtc.ListMarkTo(new DVec2(0, dotSize / 2.0f));
                    rtc.MatrixStack.Pop();
                }
            }
            success &= rtc.ListJumpTo(DVec2.Zero);
        ...
        static bool DrawCheckSystemDelay(IRtc rtc, ILaser laser, float vStage = 100, float rStage = 60)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                float angle = i * 90;
                rtc.MatrixStack.Push(DMat4.CreateRotationZ((float)(angle * Math.PI / 180.0)));
                rtc.MatrixStack.Push(DMat4.CreateTranslation(0, -offsetY, 0));
                success &= rtc.ListJumpTo(new DVec2(-5, 0));
                success &= rtc.ListMarkTo(new DVec2(5, 0));
                success &= rtc.ListJumpTo(new DVec2(3, 2));
                success &= rtc.ListMarkTo(new DVec2(5, 0));
                success &= rtc.ListMarkTo(new DVec2(3, -2));
                success &= rtc.ListJumpTo(new DVec2(0, 0));
                rtc.MatrixStack.Pop();
                rtc.MatrixStack.Pop();
                if (!success)
                    break;
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                success &= rtc.ListJumpTo(new DVec2(-rStage, 0));
                ...
                success &= rtc.ListJumpTo(new DVec2(-rStage, lineLength));
                success &= rtc.ListWait((float)usec * 1000.0f);
                success &= rtc.ListJumpTo(new DVec2(-(totalNumberOfLines / 2.0f) * increment, lineLength));
                success &= rtc.ListSpeed(jumpSpeed, markSpeed);
                for (int lineNumber = 0; lineNumber <= totalNumberOfLines; lineNumber += 2)
                {
                    success &= rtc.ListJumpTo(new DVec2((lineNumber - totalNumberOfLines / 2.0f) * increment, lineLength));
                    success &= rtc.ListMarkTo(new DVec2((lineNumber - totalNumberOfLines / 2.0f) * increment, 0.1f));
                    if (lineNumber + 1 <= totalNumberOfLines)
                    {
                        success &= rtc.ListJumpTo(new DVec2((lineNumber + 1 - totalNumberOfLines / 2.0f) * increment, 0.1f));
                        success &= rtc.ListMarkTo(new DVec2((lineNumber + 1 - totalNumberOfLines / 2.0f) * increment, lineLength));
                    }
                }
                success &= rtc.ListSpeed(vStage, markSpeed);
                success &= rtc.ListJumpTo(new DVec2(rStage, lineLength));
