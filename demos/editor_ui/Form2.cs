...
namespace Demos
{
    public partial class Form2 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Also, need to 2 instances at sirius 3 library option.
            //Core.License(out var licenseInfo);
            //Debug.Assert(licenseInfo.RtcLicenseMax == 2);

            siriusEditorControl1.MaxDeviceCounts = 2;

            int index = 0;
            // attach first device set
            {
                index = 0;
                EditorHelper.CreateDevices(out IRtc rtc, out ILaser laser, out IDInput dInExt1, out IDInput dInLaserPort, out IDOutput dOutExt1, out IDOutput dOutExt2, out IDOutput dOutLaserPort, out IPowerMeter powerMeter, out IMarker marker, index);
                siriusEditorControl1.RegisterDevices(index, rtc, laser, powerMeter, dInExt1, dInLaserPort, dOutExt1, dOutExt2, dOutLaserPort, marker);
            }
            // attach second device set
            {
                index = 1;
                EditorHelper.CreateDevices(out IRtc rtc, out ILaser laser, out IDInput dInExt1, out IDInput dInLaserPort, out IDOutput dOutExt1, out IDOutput dOutExt2, out IDOutput dOutLaserPort, out IPowerMeter powerMeter, out IMarker marker, index);
                siriusEditorControl1.RegisterDevices(index, rtc, laser, powerMeter, dInExt1, dInLaserPort, dOutExt1, dOutExt2, dOutLaserPort, marker);
        ...
        private void text_testcase(IDocument document)
        {
            ...
            {
                var text = new EntityCircularText("Segoe UI", FontStyle.Regular, EntityCircularText.TextCircularDirections.ClockWise, 30, 90,
