...
각 오프셋 인덱스에 할당된 확장 데이터(`ExtensionData`)를 개별적으로 설정하거나 조회합니다. 
- **주의**: 텍스트(혹은 바코드) 와 같은 엔티티의 `TextConverter` 속성이 `Offset`으로 설정된 경우, 이 확장 데이터에서 엔티티 이름에 해당하는 값을 자동으로 찾아 치환합니다.
- **주의**: 이 명령은 반드시 **`offset` 명령이 선행**되어 오프셋 정보가 정의된 후에 실행되어야 합니다. 또한 오프셋 개수(`Count`)와 동일해야 합니다.
- **텍스트 데이터 일괄 설정**: `text|{Count}|{Text1}|...|{Text n};`
  - `{Count}`: 설정할 오프셋의 개수 (앞서 설정된 `offset` 명령의 `{Count}`와 일치해야 함)
  - `{Text}`: 단순 문자열 또는 Key-Value 쌍 (`Name|Value`). 
...
2. **오프셋 2개 및 각 텍스트 데이터 오프셋 위치별 개별설정**:
   - `offset|2|0|0|0|0|10|10|0|0;`
   - `text|2|EntityName1|Text1|EntityName2|Text2;`
3. **텍스트 데이터 조회**:
   - `text;`
   - (응답: `Text|1|EntityName|Hello;`)
   - (응답: `Text|2|EntityName1|Text1|EntityName2|Text2;`)
