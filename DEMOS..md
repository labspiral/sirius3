...
This is a demo program that utilizes the scanner's 3D field correction feature.
Patterns are marked at regular intervals at two different Z-axis positions. These are then measured using a measurement device (e.g., machine vision), and the error values are used to generate a new calibration file from the current one.
In 3D calibration, the distortion of the Z-space (volume) is calculated using a linear stretch ratio. This demonstrates how to load the newly generated calibration file into the table and select it.

## editor_fieldcorrection_3d_calibration

This is a demo program that utilizes the scanner's 3D field correction feature.
It generates a 3D calibration file by inputting a 3D point cloud (a set of points that make up the surface of a 3D mesh).
When machining begins using 2D machining data, the laser focus automatically moves to the corresponding X,Y positions on the 3D mesh surface.
At the application level, simply providing X,Y data results in the data being projected onto a 3D surface, enabling laser machining on the 3D surface.
