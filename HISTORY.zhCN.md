...
- added) 支持 MCP（Model Context Protocol）
  - 添加 MCP 服务器，支持 AI 客户端注册编辑器、编辑文档与实体以及控制设备
- added) 支持 WPF
  - 添加 WPF 专用 UI 编辑器（`SiriusEditorControl`、`SiriusMultiEditorControl`）
  - 预览版本
- updated) 在所有目标框架中将 `Microsoft.Extensions.Logging`、`Microsoft.Extensions.Logging.Abstractions` 和 `Microsoft.Extensions.Logging.Debug` 包版本统一为 10.0.10
- fixed) 扩展多语言输出支持
- fixed) 改进异常日志，在发生异常时记录调用堆栈
- fixed) 修复 `Rtc6SyncAxis` 初始化问题
...
- 修复) 恢复属性说明中本地化时遗漏的相关设置、注意事项和处理顺序，并分行显示
- 修复) 多光束 Both 模式的 JumpAndShoot 现在会让令牌释放等待与实际跳转重叠，并只将成本更低的短跳转合并成小段处理
- 修复) 编辑器和树视图中的 CTRL+R、CTRL+M、相机、模拟和加工快捷键现在都能可靠工作，同时所有方向键组合仍保留给树导航使用
- 修复) 加工期间或通过远程桌面使用编辑器时，保持选择锁定的同时仍可缩放和平移视图
- 变更) 导入 DXF、DWG、HPGL 和 PLT 路径时，现在可通过 `Config.ImportMergeDistance` 统一设置端点合并距离
- 变更) Core/UI 配置的名称和说明现在会随所选语言显示，DXF、DWG 和 Gerber 导入也可选择保留源颜色或映射为最接近的实体加工笔颜色
...
- 修复) 填充、ALC、画笔等列表编辑器现在能更可靠地更新数值和预览
- 新增) 加工路径模拟加入固定屏幕尺寸标记、更醒目的激光束和轻量的渐隐碎屑效果
